...
# Alternative import for MNIST that doesn't require pandas
try:
    from sklearn.datasets import fetch_openml
    USE_OPENML = True
except ImportError:
...
def load_mnist_data():
    """Load and preprocess MNIST dataset from inputs directory"""
    print("Loading MNIST dataset from inputs directory...")
    ...
    try:
        ...
        train_data = pd.read_csv("inputs/MNIST-train.csv")
        print(f"Training data shape: {train_data.shape}")
        
        # Load test data
        print("Loading test data...")
        test_data = pd.read_csv("inputs/MNIST-test.csv")
        print(f"Test data shape: {test_data.shape}")
        
        # Separate features and labels using 'y' column
        X_train = train_data.drop('y', axis=1).values  # All columns except 'y'
        y_train = train_data['y'].values               # 'y' column (labels)
        ...
        print(f"Error: Could not find MNIST data files in inputs directory: {e}")
        print("Falling back to sklearn digits dataset...")
        
        # Fallback to digits dataset (smaller but similar)
        ...
        print("Using sklearn digits dataset (smaller version)")
...
def optimize_hyperparameters(X_train, y_train, X_val, y_val, max_trials=25):
    ...
    Args:
        ...
        X_val, y_val: Validation data (or test data)
    ...
    # Learning rates to examine
    learning_rates = [0.0001, 0.001, 0.01, 0.1, 0.5]
    
    # Epochs to examine
    epochs_list = [20, 30, 50, 100, 150]
    
    # Batch sizes to examine
    ...
    def generate_smart_combinations():
        ...
        for i, arch in enumerate(architectures):
            combinations.append({
                'architecture': arch,
                'learning_rate': 0.01,  # Good default
    ...
    results_dir = "outputs/optimization_results"
...
def plot_optimization_results(results, plots_dir, timestamp):
    ...
    # Architecture depth vs accuracy
    plt.subplot(2, 2, 1)
    depth_accuracy = {}
    for depth, acc in zip(architecture_depths, val_accuracies):
        if depth not in depth_accuracy:
            depth_accuracy[depth] = []
        depth_accuracy[depth].append(acc)
    
    depth_means = [np.mean(depth_accuracy[d]) for d in sorted(depth_accuracy.keys())]
    depth_stds = [np.std(depth_accuracy[d]) for d in sorted(depth_accuracy.keys())]
    plt.errorbar(sorted(depth_accuracy.keys()), depth_means, yerr=depth_stds, fmt='o-', capsize=5)
    plt.xlabel('Architecture Depth (Hidden Layers)')
    plt.ylabel('Mean Validation Accuracy')
    plt.title('Architecture Depth vs Performance')
    plt.grid(True, alpha=0.3)
    
    # Architecture width vs accuracy
    plt.subplot(2, 2, 2)
    width_accuracy = {}
    for width, acc in zip(architecture_widths, val_accuracies):
        if width not in width_accuracy:
            width_accuracy[width] = []
        width_accuracy[width].append(acc)
    
    width_means = [np.mean(width_accuracy[w]) for w in sorted(width_accuracy.keys())]
    width_stds = [np.std(width_accuracy[w]) for w in sorted(width_accuracy.keys())]
    plt.errorbar(sorted(width_accuracy.keys()), width_means, yerr=width_stds, fmt='s-', capsize=5)
    plt.xlabel('Architecture Width (Max Hidden Neurons)')
    plt.ylabel('Mean Validation Accuracy')
    plt.title('Architecture Width vs Performance')
    plt.grid(True, alpha=0.3)
    
    # Training time vs accuracy
    plt.subplot(2, 2, 3)
    scatter = plt.scatter(training_times, val_accuracies, alpha=0.7, c=total_params, cmap='plasma')
    plt.xlabel('Training Time (seconds)')
    plt.ylabel('Validation Accuracy')
    plt.title('Training Efficiency vs Performance')
    plt.colorbar(scatter, label='Total Parameters')
    plt.grid(True, alpha=0.3)
    
    # Parameter efficiency
    plt.subplot(2, 2, 4)
    efficiency = [acc / (params / 1000) for acc, params in zip(val_accuracies, total_params)]
    scatter = plt.scatter(total_params, efficiency, alpha=0.7, c=val_accuracies, cmap='viridis')
    plt.xlabel('Total Parameters')
    plt.ylabel('Accuracy per 1000 Parameters')
    plt.title('Parameter Efficiency')
    plt.colorbar(scatter, label='Validation Accuracy')
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    detailed_plot_path = os.path.join(plots_dir, f"detailed_analysis_{timestamp}.png")
...
def main():
    ...
    if best_config is None:
        print("No previous optimization results found. Using default configuration.")
    ...
    print(f"\nFinal Test Accuracy: {final_test_accuracy:.4f} ({final_test_accuracy*100:.2f}%)")
    ...
    loss_plot_path = os.path.join(plots_dir, f"final_model_training_loss_{timestamp}.png")
    ...
    for i, idx in enumerate(sample_indices):
        prediction = final_nn.predict(X_test[idx:idx+1])
...
def run_optimization():
    ...
    best_config, results = optimize_hyperparameters(X_train, y_train, X_test, y_test, max_trials=25)
    ...
    print(f"Best Test Accuracy: {best_config['val_accuracy']:.4f}")

def run_optimization_mb():
    """Separate function to run hyperparameter optimization on MB dataset"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "optimize":
            ...
            main_mb()
        elif sys.argv[1] == "optimize_mb":
            run_optimization_mb()
        else:
            print("Usage:")
            print("  python final_project.py          # Run MNIST dataset")
            print("  python final_project.py optimize # Run MNIST optimization")
            print("  python final_project.py mb       # Run MB dataset")
            print("  python final_project.py optimize_mb # Run MB optimization")
    else:
        main()
